public class sudokusolver {
    ...
    public static boolean sudokusoliving(int sudoku[][],int row,int col){
if(row==9&&col==0){
    return true;
}
...
if(sudoku[row][col]!=0){
    return sudokusoliving(sudoku, nextrow, nextcol);
}
        for(int digit=1;digit<=9;digit++){
            if(issafe(sudoku,row,col,digit)){
                sudoku[row][col]=digit;
                if(sudokusoliving(sudoku, nextrow, nextcol)){
                    return true;
                }
                sudoku[row][col]=0;
